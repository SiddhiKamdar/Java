...
public class TxMgmt {
    public static void main(String[] args) throws Exception{
        ...
        con.setAutoCommit(false); //default - commit is true
        ...
        //TX1 - atomic - transfer (ACID Properties remember!!)
        Statement st = con.createStatement();
        String withdrawq1 = "update account set accbal = accbal - "+ amount+" where accno = "+src;
        st.addBatch(withdrawq1);  // batch processing -(badhi queries ek saathe execute thase!) can add as many quries as possible in the batch 
        String depositq1 = " update account set accbal = accbal + "+ amount+" where accno = "+dest;
        st.addBatch(depositq1); // batch processing
        //execute the batch
        int[] res = st.executeBatch(); // all the queries in batch will be executed at once - returns an array 
